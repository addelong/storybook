...
def create_video_from_images_and_dialogs(images_directory, image_extension, background_music, dialog_directory, dialog_extension, dialog_texts, output_video):
    ...
    with open(temp_concat_file, "w") as concat_file:
        for i, (image, dialog, text) in enumerate(zip(image_files, dialog_files, dialog_texts)):
            ...
            "-vf", f"scale=2304:4032, zoompan=z='1+on/{segment_frames}*0.09':d={segment_frames}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':fps=30:s=768x1344, fade=t=in:st=0:d={fade_in_duration}, fade=t=out:st={float(dialog_duration)+fade_in_duration-1}:d={segment_fade_out_duration}, drawbox=y=ih-360:color=black@0.5:t=fill:width=iw:height=150, drawtext=fontfile=./CaslonAntique.ttf:text='{wrapped_text}':fontcolor=white:fontsize=34:x=(w-tw)/2:y=h-360+(lh-20)",
