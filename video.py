...
def create_video_from_images_and_dialogs(images_directory, image_extension, background_music, dialog_directory, dialog_extension, dialog_texts, output_video):
    ...
    with open(temp_concat_file, "w") as concat_file:
        for i, (image, dialog, text) in enumerate(zip(image_files, dialog_files, dialog_texts)):
            ...
            # Make dialog louder relative to background music
            "-af", f"adelay={fade_in_duration * 1000}|{fade_in_duration * 1000},volume=2.0",
            "-y", segment_file
        ])
    ...
    if background_music:
        ...
        subprocess.call([
            ...
            "-i", background_music,
            "-t", video_duration,
            "-filter_complex", f"[0:a]volume=0.06,afade=t=in:st=0:d=2,afade=t=out:st={float(video_duration)-2}:d=2[a]",
            "-map", "[a]",
            "-y", temp_music_file
        ])
